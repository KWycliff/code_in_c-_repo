...
void StartDefaultTask(void const * argument)
{
  ...
  for(;;)
  {
    xstatus = xQueueSendToBack(myQueue01Handle,5,xTicksToWait);
    if(xstatus != pdPASS){
      uint8_t buf[20] = "could not send to queue";
      HAL_UART_Transmit(&huart2,(const uint8_t*)buf,20,HAL_MAX_DELAY);
    }
    //osDelay(1);
  }
  /* USER CODE END 5 */
